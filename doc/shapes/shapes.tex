...
The polynom itself is defined with an origin at the center of $I$ : this choice is designed to reduce floating point errors during computation, as the polynom will be evaluated "as close to zero" as feasible.
\begin{center}\begin{tikzpicture}
...
Shifting does not change coefficients as the polynom is computed relative to the interval center:
\[ \Shifted{h}{P_{I,a_k}} = P_{h+I, a_k} \]

Reversion inverses the interval and odd coefficients:
\[ \Reversed{P_{I,a_k}}(x) = \sum_k a_k (-x)^k = \sum_k a_k (-1)^k x^k = P_{-I, a_k (-1)^k}(x) \]

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Convolution \& Cross-correlation}
...
\[
    ...
    \frac{1}{\sqrt{\delta}} \Shifted{(k-\frac{1}{2})\delta}{\OldIndicator{\delta}}
\]
The interval kernel is a scaled indicator function.
\[ W_l = \frac{1}{\sqrt{\eta_l}} \mathbbm{1}_{\left[ -\frac{\eta_l}{2}, \frac{\eta_l}{2} \right]} = \frac{1}{\sqrt{\eta_l}} \OldIndicator{\eta_l} \]
...
\[
    W_l \Convolution W_m =
    \frac{1}{\sqrt{\eta_l \eta_m}} \OldIndicator{\eta_l} \Convolution \OldIndicator{\eta_m} =
...
\[ \begin{split}
    ...
    & = \frac{1}{\sqrt{\eta_l \eta_m}} \Trapezoid{h}{b} \Convolution \frac{1}{\sqrt{\delta}} \Shifted{(k-\frac{1}{2})\delta}{\OldIndicator{\delta}} \\
    &= \frac{1}{\sqrt{\delta \eta_l \eta_m}} \Shifted{(k-\frac{1}{2})\delta}{\Trapezoid{h}{b} \Convolution \OldIndicator{\delta}}
\end{split} \]
We can decompose the trapezoid to easily compute the convolution:
\[ \begin{split}
    C & = \Trapezoid{h}{b} \Convolution \OldIndicator{\delta} \\
    & = \Shifted{-(h+\frac{b}{2})}{\PositiveTriangle{h}} \Convolution \OldIndicator{\delta}  + (h \times \OldIndicator{b}) \Convolution \OldIndicator{\delta} + \Shifted{h+\frac{b}{2}}{\NegativeTriangle{h}} \Convolution \OldIndicator{\delta} \\
    & = \Shifted{-(h+\frac{b}{2})}{\OldIndicator{\delta} \Convolution \PositiveTriangle{h}} + h \times (\OldIndicator{b} \Convolution \OldIndicator{\delta}) + \Shifted{h+\frac{b}{2}}{\Reversed{\OldIndicator{\delta} \Convolution \PositiveTriangle{h}}}
\end{split} \]
...
\[
    ...
    \frac{1}{\sqrt{\delta \eta_l}} \times \OldIndicator{\eta_l} \Convolution \Shifted{(k-\frac{1}{2})\delta}{\OldIndicator{\delta}} =
