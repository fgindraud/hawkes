...
static auto get_heterogeneous_kernels (const DataByProcessRegion<SortedVec<DataPoint>> & data_points,
...
determine_kernel_setup (const DataByProcessRegion<SortedVec<DataPoint>> & data_points, KernelConfig config,
			...
			if (override_homogeneous_kernel_widths.value.size () != data_points.nb_processes ()) {
				throw std::runtime_error (
				    fmt::format ("Explicit kernel widths number does not match number of processes: expected {}, got {}",
				                 data_points.nb_processes (), override_homogeneous_kernel_widths.value.size ()));
			}
